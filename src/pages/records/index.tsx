...
const Records1: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get<Transaction[]>("http://localhost:999/get-transaction");
  ...
  const handleDelete = async (id: string) => {
    try {
      await axios.delete(`http://localhost:999/delete-transaction/${id}`);
      setFetch(true);
    } catch (error) {
  ...
  return (
    <div className='dashboardContainer' style={{}}>
      ...
      <div style={{ display: "flex" }}>
        ...
        />
        <List transactions={filteredTransactions} onDelete={handleDelete} />
      </div>
    </div>
  );
};
